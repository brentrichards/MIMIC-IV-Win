...
(
  select ie.stay_id
    , min(DATE(charttime - INTERVAL '1 HOUR')) as starttime
    , max(DATE(charttime + INTERVAL '4 HOUR')) as endtime
    , max(CASE
  ...
  inner join mimic_icu.chartevents ce
    ...
    and ce.charttime between ie.intime and DATE(ie.intime + INTERVAL '1 DAY')
  where itemid = 226732
