...
(
  ...
  left join mimic_hosp.services se
    ...
    and se.transfertime < DATE(ie.intime + INTERVAL '1 DAY')
  group by ie.stay_id
)
...
(
    ...
    FROM mimic_icu.icustays ie
    LEFT JOIN mimiciv.ventilation_mv v
        ON ie.stay_id = v.stay_id
        AND (
            v.starttime BETWEEN ie.intime AND DATE(ie.intime + INTERVAL '1 DAY')
        OR v.endtime BETWEEN ie.intime AND DATE(ie.intime + INTERVAL '1 DAY')
        OR v.starttime <= ie.intime AND v.endtime >= DATE(ie.intime + INTERVAL '1 DAY')
        )
...
select ie.subject_id, ie.hadm_id, ie.stay_id
      ...
      , ie.outtime
      , adm.deathtime
      , ROUND(CAST(EXTRACT(EPOCH FROM ie.intime - adm.admittime)/60 as numeric),2) as preiculos
      , ag.age
      , gcs.gcs_min
...
left join mimiciv.first_day_vitalsign_mv vital
