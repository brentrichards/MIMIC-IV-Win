...
(
    select 
        ...
        , stay_id
        , intime AS starttime
        , DATE(intime + INTERVAL '24 HOURS') AS endtime
...
(
  select 
    co.subject_id
    , co.stay_id
    , GREATEST(min(DATE(charttime - INTERVAL '1 HOUR')), co.starttime) as starttime
    , LEAST(max(DATE(charttime + INTERVAL '4 HOUR')), co.endtime) as endtime
	, max(case when (ce.value) ~* '(cpap mask|bipap)' then 1 else 0 end) as cpap
--	, max(case when REGEXP_CONTAINS(lower(ce.value), '(cpap mask|bipap)') then 1 else 0 end) as cpap
  ...
  where ce.itemid = 226732
	and (case when (ce.value) ~* '(cpap mask|bipap)'  then 1 else 0 end)  = 1
...
(
    ...
    FROM co
    left join mimiciv.vitalsign_mv vital
...
(
    ...
    FROM co
    left join mimiciv.chemistry_mv labs
...
(
    ...
    FROM co
    LEFT JOIN mimiciv.complete_blood_count_mv cbc
...
(
    ...
    FROM co
    LEFT JOIN mimiciv.enzyme_mv enz
