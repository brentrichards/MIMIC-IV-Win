...
(
  ...
  INNER JOIN mimiciv.norepinephrine_mv mv
    ON ie.stay_id = mv.stay_id
    AND mv.starttime >= DATE(ie.intime - INTERVAL '6 HOURS')
    AND mv.starttime <= DATE(ie.intime + INTERVAL '1 DAY')
  UNION ALL
  select ie.stay_id, 'epinephrine' AS treatment, vaso_rate as rate
  FROM mimic_icu.icustays ie
  INNER JOIN mimiciv.epinephrine_mv mv
    ON ie.stay_id = mv.stay_id
    AND mv.starttime >= DATE(ie.intime - INTERVAL '6 HOURS')
    AND mv.starttime <= DATE(ie.intime + INTERVAL '1 DAY')
  UNION ALL
  ...
  FROM mimic_icu.icustays ie
  INNER JOIN mimiciv.dobutamine_mv mv
    ON ie.stay_id = mv.stay_id
    AND mv.starttime >= DATE(ie.intime - INTERVAL '6 HOURS')
    AND mv.starttime <= DATE(ie.intime + INTERVAL '1 DAY')
  UNION ALL
  ...
  FROM mimic_icu.icustays ie
  INNER JOIN mimiciv.dopamine_mv mv
    ON ie.stay_id = mv.stay_id
    AND mv.starttime >= DATE(ie.intime - INTERVAL '6 HOURS')
    AND mv.starttime <= DATE(ie.intime + INTERVAL '1 DAY')
)
...
(
  ...
  LEFT JOIN mimiciv.bg_mv bg
      ...
      AND bg.charttime >= DATE(ie.intime - INTERVAL '6 HOURS')
      AND bg.charttime <= DATE(ie.intime + INTERVAL '1 DAY')
...
left join mimiciv.first_day_vitalsign_mv v
  on ie.stay_id = v.stay_id
left join mimiciv.first_day_lab_mv l
  on ie.stay_id = l.stay_id
left join mimiciv.first_day_urine_output_mv uo
  on ie.stay_id = uo.stay_id
left join mimiciv.first_day_gcs_mv gcs
