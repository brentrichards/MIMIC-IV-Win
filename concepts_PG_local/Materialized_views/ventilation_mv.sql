...
CREATE MATERIALIZED VIEW mimiciv.ventilation_mv AS

WITH tm AS
(
  SELECT stay_id, charttime
  FROM mimiciv.ventilator_setting_mv
  UNION DISTINCT
  SELECT stay_id, charttime
  FROM mimiciv.oxygen_delivery_mv
)
, vs AS
(
  ...
  FROM tm
  LEFT JOIN mimiciv.ventilator_setting_mv vs
      ON tm.stay_id = vs.stay_id
      AND tm.charttime = vs.charttime
  LEFT JOIN mimiciv.oxygen_delivery_mv od
...
--        , DATETIME_DIFF(charttime, charttime_lag, MINUTE)/60 as ventduration
		, ROUND(cast(extract (EPOCH FROM (charttime - charttime_lag)/3600) as numeric), 2) as los_icu
...
--            WHEN DATETIME_DIFF(charttime, charttime_lag, HOUR) >= 14 THEN 1
			WHEN ROUND(cast(extract (EPOCH FROM (charttime - charttime_lag)/3600) as numeric), 2) >= 14 THEN 1
...
--            OR DATETIME_DIFF(charttime_lead, charttime, HOUR) >= 14
	  		  OR ROUND(cast(extract (EPOCH FROM (charttime - charttime_lag)/3600) as numeric), 2) >= 14 
                THEN charttime
        ELSE charttime_lead
        END
   ) AS endtime
