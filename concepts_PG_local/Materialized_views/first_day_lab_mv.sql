...
CREATE MATERIALIZED VIEW mimiciv.first_day_lab_mv AS
...
(
    ...
    LEFT JOIN mimiciv.complete_blood_count_mv le
        ON le.subject_id = ie.subject_id
        AND le.charttime >= DATE(ie.intime - INTERVAL '6 HOURS')
        AND le.charttime <= DATE(ie.intime + INTERVAL '1 DAY')
    GROUP BY ie.stay_id
)
, chem AS
(
    ...
    FROM mimic_icu.icustays ie
    LEFT JOIN mimiciv.chemistry_mv le
        ON le.subject_id = ie.subject_id
        AND le.charttime >= DATE(ie.intime - INTERVAL '6 HOURS')
        AND le.charttime <= DATE(ie.intime + INTERVAL '1 DAY')
    GROUP BY ie.stay_id
...
(
 ...
 --   , MIN(abs_neutrophils) AS abs_neutrophils_min, MAX(abs_neutrophils) AS abs_neutrophils_max
    ...
    FROM mimic_icu.icustays ie
    LEFT JOIN mimiciv.blood_differential_mv le
        ON le.subject_id = ie.subject_id
        AND le.charttime >= DATE(ie.intime - INTERVAL '6 HOURS')
        AND le.charttime <= DATE(ie.intime + INTERVAL '1 DAY')
    GROUP BY ie.stay_id
...
(
    ...
    FROM mimic_icu.icustays ie
    LEFT JOIN mimiciv.coagulation_mv le
        ON le.subject_id = ie.subject_id
        AND le.charttime >= DATE(ie.intime - INTERVAL '6 HOURS')
        AND le.charttime <= DATE(ie.intime + INTERVAL '1 DAY')
    GROUP BY ie.stay_id
...
(
    ...
    FROM mimic_icu.icustays ie
    LEFT JOIN mimiciv.enzyme_mv le
        ON le.subject_id = ie.subject_id
        AND le.charttime >= DATE(ie.intime - INTERVAL '6 HOURS')
        AND le.charttime <= DATE(ie.intime + INTERVAL '1 DAY')
    GROUP BY ie.stay_id
