...
CREATE MATERIALIZED VIEW mimiciv.kdigo_uo_mv AS
...
(
  ...
  -- 6 hours
  , sum(case when iosum.charttime >= DATE(io.charttime - interval '5 hour')
      then iosum.urineoutput
    else null end) as UrineOutput_6hr
  -- 12 hours
  , sum(case when iosum.charttime >= DATE(io.charttime - interval '11 hour')
      then iosum.urineoutput
  ...
  , ROUND(CAST(
      EXTRACT(EPOCH FROM io.charttime - 
        ...
        MIN(case when iosum.charttime >= DATE(io.charttime - interval '5 hour')
  ...
  , ROUND(CAST(
      EXTRACT(EPOCH FROM io.charttime - 
        -- below MIN() gets the earliest time that was used in the summation 
        MIN(case when iosum.charttime >= DATE(io.charttime - interval '11 hour')
          then iosum.charttime
        else null end)
        )/3600 as NUMERIC ),4)
   AS uo_tm_12hr
  , ROUND(CAST(
	  EXTRACT(EPOCH FROM io.charttime - MIN(iosum.charttime)
 --     DATETIME_DIFF(io.charttime, MIN(iosum.charttime), SECOND)
	 )/3600 as NUMERIC ),4) AS uo_tm_24hr
	--   AS NUMERIC)/3600.0, 4) AS uo_tm_24hr
  from mimiciv.urine_output_mv io
  -- this join gives all UO measurements over the 24 hours preceding this row
  left join mimiciv.urine_output_mv iosum
    on  io.stay_id = iosum.stay_id
    and iosum.charttime <= io.charttime
    and iosum.charttime >= DATE(io.charttime - interval '23 hour')
